...
SET MAINQUEST=0
SET CITY=
SET EQUIPMENTTXT=./TEXTFILES/EQUIPMENT.txt
SET EQUIPPEDSPELLSTXT=./TEXTFILES/EQUIPPEDSPELLS.txt
SET INVENTORYTXT=./TEXTFILES/INVENTORY.txt
SET SAVEDATA=./TEXTFILES/SAVEDATA.txt
SET SPELLSTXT=./TEXTFILES/SPELLS.txt
...
:SAVE
ECHO PLAYER.CLASS=%PLAYER.CLASS% > %SAVEDATA%
(
...
ECHO PLAYER.DAMAGE=%PLAYER.DAMAGE%
...
:LOAD
FOR /F "usebackq tokens=1* delims==" %%A IN (%SAVEDATA%) DO SET %%A=%%B
GOTO %CITY%

:INTRO
ECHO > %EQUIPMENTTXT%
ECHO > %EQUIPPEDSPELLSTXT%
ECHO > %INVENTORYTXT%
ECHO > %SAVEDATA%
ECHO > %SPELLS%
CALL :CLS
...
CALL :CREATECHARACTER KNIGHT,12,12,8,5
...
IF %CHOICE%==2 CALL :GREENVILLE.TAVERN.A
GOTO GREENVILLE.TAVERN

:GREENVILLE.TAVERN.A
CALL :CLS
ECHO YOU SIT DOWN AND
ECHO A MAN APPROACHES YOU FROM BEHIND
ECHO AND PUNCHES YOU
ECHO DRUNK - "HEY, DO I KNOW YOU"
CALL :CREATECHARACTER DRUNK,10,5,5,8
CALL :CREATECHARACTER.EQUIP 1,1,1
ECHO 1-HIT BACK (FIGHT)
CHOICE /C 1 /N
CALL :COMBATINIT DRUNK
EXIT /B 0
...
IF %CHOICE%==0 (
  SET "ENEMY.LASTTURN=ATTACKED YOU"
  CALL :ATTACK PLAYER,DRUNK,!%ENEMY.NAME%.WEAPON!
)
IF %CHOICE%==1 (
  SET ENEMY.LASTTURN=BLOCKED
  CALL :BLOCK ENEMY
...
:REWARD.DRUNK
SET /A %~1=%RANDOM%*4/32767+1
SET /A %~2=%RANDOM%*5/32767+1
EXIT /B 0
